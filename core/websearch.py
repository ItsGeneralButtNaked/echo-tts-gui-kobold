"""
core/websearch.py — Web search integration for Ecko.
...
# ─────────────────────────────────────────────────────────────────────────────
# RATE LIMITING + CONTEXT CAP
# ─────────────────────────────────────────────────────────────────────────────

# Minimum seconds between Brave API calls.  Prevents a burst of rapid messages
# each detected as search intent from exhausting the API key quota.
# Module-level (single user).  Move to a per-session dict if multi-user support
# is added.
SEARCH_MIN_INTERVAL: float = 3.0
_last_search_time:   float = 0.0

# Hard cap on the length of the injected results block.  Prevents a very large
# results page from blowing out the LLM context window.
...
def brave_search(query: str, api_key: str, count: int = 4) -> str:
    ...
    # Rate limit — reject calls that arrive too soon after the previous one
    elapsed = time.time() - _last_search_time
    if elapsed < SEARCH_MIN_INTERVAL:
        log.warning("[WEBSEARCH] Rate limited — %.1fs since last search (min %.1fs)",
                    elapsed, SEARCH_MIN_INTERVAL)
        return "[Web search: rate limited, please wait a moment]"
    ...
    # Cap injected context length to avoid blowing out the LLM context window
